...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MMXIV2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MMXIV2
    // Mac: ~/Library/Application Support/MMXIV2
    // Unix: ~/.MMXIV2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MMXIV2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "maieuticoind.pid"));
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("Maieuticoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
