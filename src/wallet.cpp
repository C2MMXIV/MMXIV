...
// Maieuticoin: optional setting to unlock wallet for block minting only;
...
// Maieuticoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Maieuticoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // Maieuticoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // Maieuticoin: not avatar mode
                    {
...
// Maieuticoin: create coin stake transaction
...
// Maieuticoin: check 'spent' consistency between wallet and txindex
// Maieuticoin: fix wallet spent state according to txindex
...
// Maieuticoin: disable transaction (only for coinstake)
